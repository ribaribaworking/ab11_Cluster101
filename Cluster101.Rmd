...
Let's [install](https://nf-co.re/docs/usage/installation) the Nextflow using Conda (Mamba):
...
Make sure that `ABSOLUTE_PATH_TO_YOUR_PROJECT_FOLDER` is replaced with the absolute path to your project folder (should look like this: `~/path/to/your/project.folder/` ). This csv table specifies the names of samples and provides paths to fastq files.
...
This is going to be an executable file. At the beginning of the file, we change the directory to a `Data_processed`, to not pollute the `src` folder with temporary files that NextFlow is going to produce. Also, we specify the variables with paths to necessary files - genome files, indexes, etc. We already have a big collection of built indexes and reference genomes, so before loading it check the Public folder, maybe we already have what you need. You can read more about the parameters [here](https://nf-co.re/rnaseq/3.12.0/docs/usage).

Now, everything should be ready for the run. To src folder and run bash `01_run_RNAseq_ppln.sh` Don't forget to run it from tmux. You should see the start of the pipeline. It might take around an hour to complete the run. As a result, you will have a lot of different files in your `Data_Processed` folder. You will have a work folder, which contains a lot of temporary files, that most likely you don't want to track either with DVC or with Git. So, you need to add to `.gitignore` and to `.dvcignore` the following pattern:

`*/work/*`

Now, it's a good idea to stage the results with DVC by typing:

`dvc add Data_Processed`

Great! Now, we successfully pre-processed our data and can go to the next step - analysis of the results in R.
